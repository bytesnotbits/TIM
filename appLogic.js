...
function handleInventoryListClick(event) {
    ...
    if (target.matches('button[data-action="flag"]')) {
        ...
        console.log("[handleInventoryListClick] 'View History' button matched.");
         // Fetch item details to get a reliable SKU and Description for the title, even if not rendered
        findInventoryItemByItemId(itemId).then(item => {
            const displaySku = item ? item.SKU : sku; // Fallback to dataset SKU if item not found
            ...
            try {
                 showItemHistory(itemId, displaySku, displayDesc); // Pass desc too
            } catch (historyError) {
                 console.error(`Error opening history modal for ${itemId}:`, historyError);
                 alert("Failed to open item history modal. See console.");
            }
        }).catch(err => {
        ...
        // Find the corresponding input within the same itemDiv
        const inputElement = itemDiv.querySelector(`input[data-sequence="${sequenceType}"]`);

        if (inputElement && !inputElement.disabled) {
            console.log(`Applying value '${sequenceValue}' to input data-sequence='${sequenceType}' for item ${itemId}`);
            inputElement.value = sequenceValue; // Set the input value

            // --- CRITICAL: Trigger the change event ---
            // This makes the application react as if the user manually typed and blurred
            console.log(`Dispatching 'change' event for input data-sequence='${sequenceType}'`);
            inputElement.dispatchEvent(new Event('change', { bubbles: true }));

        } else if (!inputElement) {
             console.error(`Could not find input element for sequence type '${sequenceType}' in item ${itemId}`);
        } else {
             console.warn(`Input element for sequence type '${sequenceType}' is disabled.`);
        }
...
async function updateCount(itemId, quantityStr) {
    ...
    if (isNaN(quantity) || quantity < 0) {
        ...
        applyCurrentFilters(); // Re-render to reset the invalid input
        return;
    }
...
function calculateFootageForItem(item, sequences) {
    ...
    try {
        let totalDifference = 0; // Use difference to handle potential sequence direction issues
        let calculationPossible = false;
        let errorInPair = false; // Track if a pair is entered but invalid

        // Function to parse and validate a pair
        const processPair = (innerStr, outerStr) => {
            const innerValStr = String(innerStr || '').trim();
            const outerValStr = String(outerStr || '').trim();

            // Only proceed if BOTH are entered
            if (innerValStr === '' && outerValStr === '') {
                return { difference: 0, possible: false, error: false }; // No input for this pair
            }
            if (innerValStr === '' || outerValStr === '') {
                 console.warn(`Incomplete sequence pair for itemId ${item.itemId}: Inner=${innerValStr}, Outer=${outerValStr}`);
                 return { difference: 0, possible: false, error: true }; // Incomplete pair is an error
            }

            const inner = Number(innerValStr);
            const outer = Number(outerValStr);

            if (isNaN(inner) || isNaN(outer) || inner < 0 || outer < 0) {
                 console.warn(`Invalid sequence values (non-numeric or negative) for itemId ${item.itemId}: Inner=${innerValStr}, Outer=${outerValStr}`);
                 return { difference: 0, possible: false, error: true }; // Invalid number is an error
            }

            // Allow either direction, use absolute difference
            return { difference: Math.abs(outer - inner), possible: true, error: false };
        };

        // Process First pair
        const pair1Result = processPair(sequences.inner1, sequences.outer1);
        if (pair1Result.error) errorInPair = true;
        if (pair1Result.possible) {
            totalDifference += pair1Result.difference;
            calculationPossible = true;
        }

        // Process Second pair (only if two-way reel)
        if (!errorInPair && item.isTwoWayReel) {
            const pair2Result = processPair(sequences.inner2, sequences.outer2);
            if (pair2Result.error) errorInPair = true;
            if (pair2Result.possible) {
                totalDifference += pair2Result.difference;
                calculationPossible = true; // Mark as possible if pair 2 was calculated, even if pair 1 wasn't
            }
        }

        // Return calculated footage only if:
        // 1. No pair had an error (e.g., one value entered but not the other, or non-numeric)
        // 2. At least one pair was successfully processed.
        if (!errorInPair && calculationPossible) {
            return totalDifference * item.footageFactor;
        } else {
            return null; // Calculation is invalid due to errors or no valid pairs entered
        }
...
async function updateSequences(itemId) {
    ...
    try {
        ...
        if (!item || !item.isActive || !item.isReel || !item.toCount) {
            ...
            // Maybe reset inputs if needed? Or just do nothing.
            applyCurrentFilters(); // Re-render to ensure UI consistency
            return;
        }

        // Find the specific item's div in the DOM to get input values
        const itemDiv = document.querySelector(`.inventory-item[data-item-id="${itemId}"]`);
        if (!itemDiv) {
             console.error(`Could not find item div for itemId ${itemId} to read sequence inputs.`);
        ...
        if (calculatedFootage !== null) {
            ...
            // If recordOrUpdateCount failed (e.g., item finished), updatedItem will be null.
            // AutoSave is handled within recordOrUpdateCount's success path.
            if (!updatedItem) {
                 console.warn(`Sequence calculation successful for ${itemId}, but count update failed or was disallowed.`);
                 // Save sequence changes anyway? Yes, autoSave below handles it.
            ...
            console.log(`Sequences updated for ${itemId}, but calculation invalid or incomplete. Saving sequence data only.`);
             ...
             // Let's leave the main 'counted' field as it was previously, only save sequences.
             // The 'flagUncounted' action should be used to explicitly reset the count.
...
async function updateItemNotes(itemId, notes) {
    ...
    try {
        ...
        if (!item || !item.isActive || !item.toCount) {
            console.warn(`Cannot update notes: Item ${itemId} not found, inactive, or already finished.`);
            // Re-render to reset textarea if needed?
             applyCurrentFilters();
            return;
        }
...
// --- UI Rendering ---
// MODIFIED renderInventoryList to improve "no items" messages and add indicators
/* function renderInventoryList() {
    const container = document.getElementById('inventoryList');
    if (!container) { console.error("Inventory list container not found."); return; }

    // ***** MODIFIED ***** Added a try...catch block here
    try {
        ...
        if (currentInventory.length === 0) {
            // ... (Keep the improved 'no items' message logic from the previous version) ...
            let message = 'No items match the current criteria.'; // Generic default
        ...
        currentInventory.forEach(item => {
            try {
                ...
                if (item.isReel) {
                    // ***** RESTORED HELPER FUNCTION LOGIC *****
                    const createSequenceDisplaySpan = (type, value) => {
                        ...
                        const lowerType = type.toLowerCase().replace('2',''); // 'inner' or 'outer' for data-sequence-type
                        const actualType = type; // 'Inner', 'Outer', 'Inner2', 'Outer2' for data-sequence-type
                        ...
                        if (displayValue === '---') {
                            ...
                            return `<span class="captured-sequence-display"
                                          data-action="apply-sequence"
                                          data-sequence-type="${actualType.toLowerCase()}"  
                                          data-sequence-value="${displayValue}"
                                          title="Click to apply '${displayValue}' to the input below">${actualType}: ${displayValue}</span>`;
                        }
                    };
                    // ***** END OF RESTORED HELPER FUNCTION LOGIC *****

                    const disableSequenceInput = !item.isActive || !item.toCount;

                    // Pair 1 - Use helper for display spans
                    // Note: We pass the RAW sequence value (item.innerSequence etc.) to the helper.
                    // The value attribute of the INPUT tag gets the same value.
                    columns.sequences1.innerHTML = `
                        <div class="sequence-group">
                            ${createSequenceDisplaySpan('Inner', item.innerSequence)}
                            <input type="number" data-sequence="inner" value="${item.innerSequence ?? ''}" placeholder="Inner" min="0" ${disableSequenceInput ? 'disabled' : ''}>
                        </div>
                        <div class="sequence-group">
                             ${createSequenceDisplaySpan('Outer', item.outerSequence)}
                            <input type="number" data-sequence="outer" value="${item.outerSequence ?? ''}" placeholder="Outer" min="0" ${disableSequenceInput ? 'disabled' : ''}>
                        </div>
                        <span class="sequence-equals">=</span>`;

                    // Pair 2 (Only if two-way) - Use helper for display spans
                    if (item.isTwoWayReel) {
                        columns.sequences2.innerHTML = `
                            <div class="sequence-group">
                                 ${createSequenceDisplaySpan('Inner2', item.innerSequence2)}
                                <input type="number" data-sequence="inner2" value="${item.innerSequence2 ?? ''}" placeholder="Inner2" min="0" ${disableSequenceInput ? 'disabled' : ''}>
                            </div>
                            <div class="sequence-group">
                                 ${createSequenceDisplaySpan('Outer2', item.outerSequence2)}
                                <input type="number" data-sequence="outer2" value="${item.outerSequence2 ?? ''}" placeholder="Outer2" min="0" ${disableSequenceInput ? 'disabled' : ''}>
                            </div>
                            <span class="sequence-equals">=</span>`;
                             columns.sequences2.style.visibility = 'visible'; // Ensure visible if two-way
                    } else {
                        columns.sequences2.innerHTML = ''; // Clear content
                        columns.sequences2.style.visibility = 'hidden'; // Hide column
                    }

                    // Append Total Calculated Footage and Factor (logic unchanged)
                    ...
                    totalFootageDisplay.textContent = (item.calculatedFootage !== null) ? `Total: ${item.calculatedFootage.toFixed(2)} ft` : 'Total: ---';
                     if (item.calculatedFootage === null && (item.innerSequence || item.outerSequence || item.innerSequence2 || item.outerSequence2)) {
                    ...
                    // Hide sequence columns if not a reel (logic unchanged)
                    columns.sequences1.innerHTML = '';
                ...
                // --- Append columns (logic unchanged) ---
                ...
                // ... (error handling for single item render - unchanged) ...
...
function renderInventoryList() {
    ...
    try { // Outer try block for the whole function
        ...
        // ***** CORRECTED ***** Declare fragment only ONCE here
        const fragment = document.createDocumentFragment();
        ...
        // ***** REMOVED ***** The duplicate declaration of 'fragment' was here.
        currentInventory.forEach(item => {
            ...
            try {
                ...
                // --- Populate Sequences Columns ---
                if (item.isReel) {
                    // Helper function for clickable sequence display
                    const createSequenceDisplaySpan = (type, value) => {
                        const displayValue = (value !== null && value !== undefined && String(value).trim() !== '') ? String(value).trim() : '---';
                    ...
                    // Append Total Calculated Footage and Factor
                    const totalFootageDisplay = document.createElement('span');
                    totalFootageDisplay.className = 'calculated-footage-display total-footage';
                    totalFootageDisplay.textContent = (item.calculatedFootage !== null) ? `Total: ${item.calculatedFootage.toFixed(2)} ft` : 'Total: ---';
                     if (item.calculatedFootage === null && (item.innerSequence || item.outerSequence || item.innerSequence2 || item.outerSequence2)) {
            ...
            try {
                ...
                parsedData.forEach((row, index) => {
                    ...
                    // --- Handle Flags based on Context ---
                     ...
                     // For 'update' context, flags are preserved unless count was updated
                     // It makes sense that if a count is provided via update CSV, it shouldn't remain 'toCount' maybe?
                     // Let's stick to preserving flags for 'update' context for now.

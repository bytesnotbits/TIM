...
| `ghApplyButtonGating()` | **Single owner of the Sync/Push buttons' enabled state** (v2.57.02). Testing mode disables both Sync buttons (`ghSyncNowBtn`, `invProgressSyncBtn`) so turning the mode off is a deliberate act by the person at the device, not a side effect of clicking Sync — a forcing function the banner alone wasn't, sitting in the way at the moment someone goes looking for their data. Push is left clickable (it refuses, explains, and re-asserts the banner). **The automatic boot pull is deliberately NOT gated** — testing mode exists to work against live current data while suppressing push-back, so blocking pulls would defeat it and leave the device drifting. Both `ghSyncNow` and `ghPushToGitHub` call this from `.finally()` instead of re-enabling unconditionally, which would otherwise un-gate the button the moment a boot sync completed |
